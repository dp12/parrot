...
You can also append to the default dictionary with something like:
#+begin_src emacs-lisp
(setq parrot-rotate-dict
      (append '((:rot ("hakuna" "matata"))
                (:rot ("peeple" "sheeple")))
              parrot-rotate-dict))
#+end_src
** Party Parrots
...
*** Customization
Is parrot not rotating as much as you want it to? To truly become a parrot provocateur, you can make parrot rotate for many divers reasons by adding =parrot-start-animation= to different hooks.
